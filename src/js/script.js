class Hover {
    ...
    hover() {
        this.target.addEventListener('mousemove', (e) => {
            this.num.style.transition = this.card.style.transition = this.cost.style.transition = this.price.style.transition = this.img.style.transition = this.card.childNodes[1].style.transition = this.cost.style.transition = `all .1s`
    ...
    unhover() {
        this.target.addEventListener('mouseout', () => {
            this.price.style.transform = this.cost.style.transition = this.img.style.transition = this.card.style.transition = `all .23s`
            this.card.style.transform = this.num.style.transform = this.img.style.transform = `rotateX(0) rotateY(0)`
            this.card.childNodes[1].style.opacity = this.price.style.opacity = this.num.style.opacity = '0.4'
            this.card.childNodes[1].style.color = 'white'
            this.card.childNodes[1].style.transform = this.price.style.transform = this.cost.style.transform = this.num.style.opacity = 'scale(1)'
...
function creatNewObject(realCard) {
    const card =  new Hover(realCard, realCard.childNodes[7], realCard.childNodes[1], realCard.childNodes[5].childNodes[1], realCard.childNodes[5], realCard.childNodes[3]) 
    card.hover(), card.unhover()
}
creatNewObject(document.querySelector('.cardholder__realCard'))
